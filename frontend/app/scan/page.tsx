...
// import Chatbot from "../summarize/chatbox";
...
const Scan = () => {
  ...
  const handlePdfFileSubmit = async (e) => {
    ...
    // if (req.method === 'POST') {
      ...
      //   const greenTick = await sharp('green-tick.png').resize(50, 50).toBuffer(); // Adjust the path and size as needed
      //   const processedImage = await sharp(file.data)
      //     .composite([{ input: greenTick, gravity: 'southeast' }])
      //     .toBuffer();
  
      //   // Save or upload processedImage as needed
      //   // For simplicity, let's just return the URL
      //   // res.status(200).json({ url: `data:image/png;base64,${processedImage.toString('base64')}` });
      // } catch (error) {
  ...
  return (
    <>
      <Breadcrumb pageName="Digitally Sign Documents" description="" />
